...
def load_table_zones_taxi(conn):
    ...
    execute_query(conn, "CREATE TEMPORARY TABLE my_temp_table (my_geojson VARIANT)")
    execute_query(conn, "COPY INTO my_temp_table (my_geojson) FROM @Zones_Taxi_STAGING/{file_name}.gz FILE_FORMAT = (FORMAT_NAME = GEOJSON_FORMAT) ON_ERROR = 'continue'")
    ...
    sqlll = fr"INSERT INTO taxi_zones (the_geom) SELECT PARSE_JSON(GET(my_geojson, 'features[1].geometry.coordinates')) FROM my_temp_table"
    execute_query(conn, sqlll)
